...
const generateInsights = (transactions) => {
    ...
    for (const transaction of expenses) {
        ...
        if (date.getMonth() === currentMonth && date.getFullYear() === currentYear) {
            monthCategoryTotals.set(transaction.category, (monthCategoryTotals.get(transaction.category) ?? 0) + transaction.amount);
        }
    }
    const topCategoryEntry = [...monthCategoryTotals.entries()].reduce((best, entry) => {
    ...
    const mostExpensiveDayEntry = [...dayTotals.entries()].reduce((best, entry) => {
        if (!best || entry[1] > best[1]) {
            return entry;
        }
        return best;
